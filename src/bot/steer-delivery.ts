...
export function isRejectedSteer(error: unknown): boolean {
  return error instanceof JsonRpcError && /expected active turn id|expected turn id|no active turn|turn already completed|turn not found|turn.*not active|turn.*mismatch/i.test(error.message);
}
