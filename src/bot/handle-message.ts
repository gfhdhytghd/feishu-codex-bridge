...
export function createOrchestrator(
  ...
  const discuss = new Discuss(`${paths.sessionsFile}.discuss.json`,
    ...
    deliver: async (key, messages, action, snapshot, context) => {
      ...
      const perm = turnSession(msg.chatId, project, msg.senderId);
      const ingestedMessages = await Promise.all(messages.map(m => ingestContext(m, m.content)));
      const bodies = messages.map((m, i) => `[discuss-delivery:${m.messageId}]\n发送者 ${m.senderName || m.senderId} (${m.senderId})\n${ingestedMessages[i]!.text}`);
  ...
  const voiceInputs = new WeakMap<NormalizedMessage, Promise<IngestedContext>>();
  function voiceInput(msg: NormalizedMessage): Promise<IngestedContext> {
    let pending = voiceInputs.get(msg);
    if (!pending) {
      pending = ingestVoice(channel, msg, voice);
      voiceInputs.set(msg, pending);
    }
    return pending;
  }
  async function voiceText(msg: NormalizedMessage, signal?: AbortSignal): Promise<string> {
    const combined = signal ? AbortSignal.any([signal, voiceShutdown.signal]) : voiceShutdown.signal;
    combined.throwIfAborted();
    const result = await voiceInput(msg);
    combined.throwIfAborted();
  ...
  const cancelContext = (key: string): void => {
    ...
    if (count && target) void channel.send(target.chatId,
      { markdown: `⚠️ ${count} 条尚未提交的消息已取消（会话已停止或设置已变更），请按需重发。` },
  ...
  async function ingestContextImpl(msg: NormalizedMessage, text: string): Promise<IngestedContext> {
    const ingested = messageHasVoice(msg) ? await voiceInput(msg) : { text };
    // Prepared inputs already contain the transcript and delivery marker.
    const inputText = messageHasVoice(msg) && text === msg.content ? ingested.text : text;
    let body = inputText;
   ...
   */
  function handleTurn(msg: NormalizedMessage, text: string, sessionKey: string, flat: boolean, project: Project | undefined, perm: TurnPerm, prepared?: { text: string; voice?: VoiceReply; images?: string[]; receipt: ContextReceipt; signal?: AbortSignal }, voicePrepared = false, preparationSignal?: AbortSignal): Promise<void> {
    ...
    return orderTurns(sessionKey, async () => {
      ...
      await prepareTurn(msg, text, sessionKey, flat, project, perm, prepared, voicePrepared, preparationSignal);
    });
  }
  ...
  async function prepareTurn(
    ...
    project: Project | undefined,
    perm: TurnPerm,
    prepared?: { text: string; voice?: VoiceReply; images?: string[]; receipt: ContextReceipt; signal?: AbortSignal },
    voicePrepared = false,
    ...
    if (!prepared && !voicePrepared && (messageHasVoice(msg) || voiceIntake.hasPending(sessionKey))) {
      ...
      voiceIntake.submit(sessionKey, async signal => {
        ...
        await prepareTurn(msg, body, sessionKey, flat, fresh, turnPerm(fresh, msg.senderId), undefined, true, signal);
    ...
    if (discussEnabled(project) && !prepared) {
      ...
      contextIntake.submit(sessionKey, async signal => {
        ...
        signal.throwIfAborted();
        const deliveryText = weaveMemoryContext(history + body.text, context.block);
        assertDiscussDeliveryBudget(deliveryText);
        return { signal, text: deliveryText, voice: body.voice, images, receipt: context.receipt };
    ...
    if (briefings && !prepared) {
      ...
      contextIntake.submit(sessionKey, async (signal) => {
        ...
        return { signal, text: weaveMemoryContext(body.text, context.block), voice: body.voice, images, receipt: context.receipt };
      }, async (input) => {
    ...
    if (existing) {
      ...
      const woven: IngestedContext = prepared ? { text: prepared.text, voice: prepared.voice } : await ingestContext(msg, text);
      if (preparationSignal?.aborted || existing.intakeCancelled) { prepared?.receipt.rejected(); return; }
      ...
      if (!cur) {
        startReservedRun(msg, woven.text, sessionKey, flat, project, perm, images, woven, text, undefined, prepared?.receipt);
        return;
      }
      ...
      void deliverPreparedTurn(msg, woven, images, text, sessionKey, flat, project, perm, cur, prepared?.receipt, preparationSignal)
        .catch((err) => log.fail('intake', err));
      return;
    }

    if (prepared) startReservedRun(msg, prepared.text, sessionKey, flat, project, perm, prepared.images, { text: prepared.text, voice: prepared.voice }, text, undefined, prepared.receipt);
  ...
  async function deliverPreparedTurn(
    ...
    sessionKey: string, flat: boolean, project: Project | undefined, perm: TurnPerm, cur: ActiveState, receipt?: ContextReceipt, signal?: AbortSignal,
  ): Promise<void> {
    if (signal?.aborted || receipt?.signal?.aborted) { receipt?.rejected(); finishEarly(msg); return; }
    if (cur.queue.length === 0 && getPendingPolicy(cfg) === 'steer' && cur.run && cur.thread && cur.thread.supportsSteer !== false) {
      ...
      if (tid) {
        ...
        try {
          await steerWithDeadline(cur.thread, { text: woven.text, images }, tid, signal ?? receipt?.signal);
          receipt?.accepted(sessionKey, cur.thread.sessionId);
          finishEarly(msg);
          ...
          if (!isRejectedSteer(err)) {
            receipt?.rejected(); finishEarly(msg);
            if (signal?.aborted || receipt?.signal?.aborted) return;
    ...
    if (cur.intakeCancelled || signal?.aborted || receipt?.signal?.aborted) { receipt?.rejected(); finishEarly(msg); return; }
    ...
    startReservedRun(msg, woven.text, sessionKey, flat, project, perm, images, woven, text, undefined, receipt);
  }
    ...
    void withTrace({ chatId: msg.chatId, msgId: msg.messageId }, async () => {
      ...
      try {
        ...
        if (!thread) {
          ...
          const be = backendFor(project?.backend);
          thread = await be.startThread({ cwd, model, effort, fastMode: project?.defaultFastMode, mode: perm.mode, network: perm.network, autoCompact: perm.autoCompact, ...(discussEnabled(project) ? { historyMode: 'legacy' as const } : {}) });
  ...
  async function resolveThread(
    ...
    if (!pending) {
      ...
      pending = resolveThreadOnce(threadId, chatId, perm, resolutionGuard(threadId), refreshPreferences);
  ...
  function startTopicDirectly(msg: NormalizedMessage, text: string, project?: Project, goal?: boolean): void {
    ...
    void withTrace({ chatId: msg.chatId, msgId: msg.messageId }, async () => {
      ...
      const threadP = (async () => {
        ...
        const thread = await be.startThread({ cwd, model, effort, fastMode: project?.defaultFastMode, mode: perm.mode, network: perm.network, autoCompact: perm.autoCompact, ...(discussEnabled(project) ? { historyMode: 'legacy' as const } : {}) });
        tResolveDone = Date.now();
      ...
      const ingestP = (async () => {
        ...
        return { ...body, text: weaveMemoryContext(body.text, context.block) };
      })();
