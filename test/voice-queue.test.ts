...
vi.mock('../src/voice/inbound', async original => ({ ...await original<object>(), ingestVoice: async (_channel: unknown, msg: NormalizedMessage) => { const text = await fake.transcribe(); return { text, voice: { messageId: msg.messageId, text, transcribed: true } }; } }));
...
vi.mock('../src/card/run-card-stream', () => ({
  RunCardStream: class {
    ...
    async drain() {}
    setImageWorker() {}
    async updateLiveCard() {}
    attachImages() {}
    async settleImages() { return new Map(); }
    getCardId() { return undefined; }
...
describe('voice queue integration', () => {
  it('steers transcribed voice with the upstream delivery path, retaining sender', async () => {
    ...
    const input = run.t.steer.mock.calls[0]![0];
    expect(input.text).toContain('语音消息：检查报表');
    expect(input.text).toContain('owner');
    expect(input.text).not.toContain('<audio');
  });
  it('holds later text behind slow voice preparation without blocking onMessage', async () => {
    ...
    const run = thread(); fake.backend.resumeThread.mockResolvedValue(run.t);
    const o = setup('queue'); await o.onMessage(voice()); await o.onMessage(message('later'));
